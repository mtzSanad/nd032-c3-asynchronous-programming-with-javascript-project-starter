  ...
  try {
    return new Promise((resolve) => {
      const runRaceInterval = setInterval(async () => {
        //Getting the race status
        const curretRace = await getRace(raceID);
        const { status, positions } = curretRace;
        if (status === "in-progress") {
          ...
          resolve(curretRace); // resolve the promise
        }
      }, 500);
    });
  } catch (error) {
  ...
  try {
    ...
    return new Promise((resolve) => {
      const countDownInterval = setInterval(() => {
        //Check if the timer is zero end the count down
        if (timer === 0) {
...
function handleSelectPodRacer(target) {
  ...
  console.log("selected a pod", playerId);
...
function getTracks() {
  ...
  return fetch(`${SERVER}/api/tracks`)
    ...
    .then((tracks) => tracks)
    .catch((err) => console.log("Problem with Getting tracks request::", err));
}
...
function getRacers() {
  ...
  return fetch(`${SERVER}/api/cars`)
    .then((response) => response.json())
    .then((cars) => cars)
    .catch((err) => console.log("Problem with Getting cars request::", err));
}
...
function getRace(id) {
  ...
  return fetch(`${SERVER}/api/races/${id}`)
    .then((response) => response.json())
    .then((races) => races)
    .catch((err) => console.log("Problem with Getting Races request::", err));
}
